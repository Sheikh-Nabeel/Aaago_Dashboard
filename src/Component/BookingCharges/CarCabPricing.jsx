...
const CarCabPricing = ({ data, onDataUpdate }) => {
  ...
  const handleSaveCarCabPricing = async (formData) => {
    try {
      await PricingUpdateService.updateServiceTypePricing('carCab', formData);
      if (onDataUpdate) {
        onDataUpdate();
      }
    } catch (error) {
  ...
  const carCabData = data?.serviceTypes?.carCab;
  const vehicleTypes = carCabData?.vehicleTypes || {};

  if (!carCabData) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-[#DDC104] text-lg">Car/Cab pricing data not available</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Service Overview */}
      <PricingCard title="Car/Cab Service Overview">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InfoRow label="Service Enabled" value={carCabData.enabled ? 'Yes' : 'No'} />
          <InfoRow label="Minimum Fare" value={carCabData.minimumFare} suffix=" AED" />
          <InfoRow label="Vehicle Types" value={tabs.length} />
        </div>
      </PricingCard>
      ...
      <ServiceEditModal
        ...
        title="Edit Car/Cab Pricing Configuration"
        data={carCabData}
        onSave={handleSaveCarCabPricing}
        serviceType="carCab"
      />
    </div>
  );
